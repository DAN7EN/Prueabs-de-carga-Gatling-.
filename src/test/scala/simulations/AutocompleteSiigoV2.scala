...
/*
* En esta simulacion los datos estan en un archivo .csv y se crea una json como plantilla
* en donde los valores del json se toman aleatorimente de las columnas del .csv
* los value en el json deben ser lo mismos nombres de las columnas en el csv.
* */
class AutocompleteSiigoV2 extends Simulation{
...
//Metodo para leer el token guardado en el archivo .txt llamado token, el cual se genera en la simulacion GetTokenSiigoV2
  ...
  val scnAutoComplete = scenario("Buscador de Siigo")
    ...
    .exec(http("Buscador de Siigo")
      ...
      .body(ElFileBody("src/test/scala/config/datosAutocomplete.json")).asJson
